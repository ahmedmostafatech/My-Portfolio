const WORKS = [
  /* ══ CREATIVE → BRANDING ══ */
  {
    id: 1,
    l1: "creative",
    l2: "branding",
    featured: true,
    num: "01",
    wide: true,
    thumb: "assets/images/VIORA Hero Brand Presentation.webp",
    title: "VIORA Brand Identity",
    subtitle: "Brand Identity",
    desc: "وصف قصير بيظهر على الكارد",
    tags: ["Logo", "Brand System", "Guidelines"],
    bg: "#0f1628",
    label: "PROJ",
    // ── بيانات الـ modal ──
    year: "2026",
    role: "Brand Designer",
    client: "VIORA",
    fullDesc:
      "وصف تفصيلي للمشروع — اكتب هنا كل حاجة عايز تقولها عن المشروع، التحديات، الحلول، النتايج.",
    images: [
      "assets/images/VIORA Hero Brand Presentation.webp",
      "assets/images/VIORA Logo Showcase.webp",
      "assets/images/VIORA Brand Applications.jpg",
      "assets/images/VIORA Luxury Brand Identity Moodboard.webp",
      "assets/images/VIORA Luxury Perfume Bottle Mockups.webp",
      "assets/images/VIORA Packaging System.webp",
      "assets/images/VIORA_Luxury_Perfume_Brand_Moodboard_Navy.webp",
      "assets/images/VIORA_Luxury_Perfume_Brand_Moodboard.webp",
    ],
    link: "", // لو في لينك خارجي
  },
  {
    id: 2,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/Brand Identity & Guidelines.webp",
    featured: true,
    num: "02",
    wide: false,
    title: "Egypt Holiday Aswan Brand Identity",
    subtitle: "Brand Identity",
    desc: "وصف قصير",
    tags: ["Logo", "Packaging", "Print"],
    bg: "#1a1008",
    label: "PROJ",
    year: "2026",
    role: "Brand Designer",
    client: "Egypt Holiday Aswan",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/Brand Identity & Guidelines.webp'],
    link: "",
  },
  {
    id: 3,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/المود بورد.webp",
    featured: true,
    num: "03",
    wide: false,
    title: "TIQ TAQ Visual Identity",
    subtitle: "Visual Identity",
    desc: "وصف قصير",
    tags: ["Brand", "Type", "Color"],
    bg: "#0a1020",
    label: "PROJ",
    year: "2025",
    role: "Barnd Designer",
    client: "TIQ TAQ",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/المود بورد.webp'],
    link: "",
  },
    {
    id: 4,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/العالمية (1).webp",
    featured: true,
    num: "04",
    wide: true,
    title: "هوية بصرية لشركة العالمية",
    subtitle: "Visual Identity",
    desc: "وصف قصير",
    tags: ["Brand", "Type", "Color"],
    bg: "#0a1020",
    label: "PROJ",
    year: "2025",
    role: "Barnd Designer",
    client: "الشركة العالمية",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/العالمية (1).webp'],
    link: "",
  },
  {
    id: 5,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/الأصيل.webp",
    featured: true,
    num: "05",
    wide: false,
    title: "هوية بصرية لبراند الأصيل",
    subtitle: "Visual Identity",
    desc: "وصف قصير",
    tags: ["Brand", "Type", "Color"],
    bg: "#0a1020",
    label: "PROJ",
    year: "2025",
    role: "Barnd Designer",
    client: "الأصيل",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/الأصيل.webp'],
    link: "",
  },
  {
    id: 6,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/EELU Brand Board 1.webp",
    featured: true,
    num: "06",
    wide: false,
    title: "EELU Aswan SB Visual Identity",
    subtitle: "Visual Identity",
    desc: "وصف قصير",
    tags: ["Brand", "Type", "Color"],
    bg: "#0a1020",
    label: "PROJ",
    year: "2025",
    role: "Barnd Designer",
    client: "EELU Aswan SB",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/EELU Brand Board 1.webp'],
    link: "",
  },
    {
    id: 7,
    l1: "creative",
    l2: "branding",
    thumb: "assets/images/Abdelkader 1.webp",
    featured: true,
    num: "07",
    wide: false,
    title: "هوية بصرية لـ آل عبد القادر للذهب والمجوهرات",
    subtitle: "Visual Identity",
    desc: "وصف قصير",
    tags: ["Brand", "Type", "Color"],
    bg: "#0a1020",
    label: "PROJ",
    year: "2025",
    role: "Barnd Designer",
    client: "آل عبد القادر",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/Abdelkader 1.webp',
      'assets/images/Abdelkader 2.webp',
      'assets/images/Abdelkader 3.webp',
    ],
    link: "",
  },

  /* ══ CREATIVE → SOCIAL ══ */
  {
    id: 8,
    l1: "creative",
    l2: "social",
    thumb: "assets/images/آل عبد القادر 2.webp",
    featured: true,
    num: "01",
    wide: true,
    title: "كامبين آل عبد القادر للذهب والمجوهرات",
    subtitle: "Social Media campaign",
    desc: "وصف قصير",
    tags: ["Facebook", "Templates", "Social Media"],
    bg: "#1a0a1a",
    label: "PROJ",
    year: "2026",
    role: "Social Media Designer",
    client: "آل عبد القادر",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/12.webp',
      'assets/images/13.webp',
      'assets/images/19.webp',
      'assets/images/22.webp',
      'assets/images/26.webp',
      'assets/images/34.webp',
      'assets/images/45.webp',
      'assets/images/50.webp',
      'assets/images/18.webp',
      'assets/images/17.webp',
      'assets/images/10.webp',
      'assets/images/12(1)(1).webp',
      'assets/images/12(1).webp',
      'assets/images/آل عبد القادر.webp',
      'assets/images/433.webp',
      'assets/images/تهنئة عيد الأضحى.webp',
      'assets/images/فيسبوك كفر 2jpg.webp',
    ],
    link: "",
  },

  {
    id: 9,
    l1: "creative",
    l2: "social",
    thumb: "assets/images/portfolio_hero_section.webp",
    featured: true,
    num: "02",
    wide: false,
    title: " Social Media Posts MR.Abdelkader",
    subtitle: "Social Media Posts",
    desc: "وصف قصير",
    tags: ["Facebook", "Posts", "Social Media"],
    bg: "#1a0a1a",
    label: "PROJ",
    year: "2026",
    role: "Social Media Designer",
    client: "م/عبد القادر الصايغ",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/1 (3).webp',
      'assets/images/3.webp',
      'assets/images/5 (1).webp',
      'assets/images/6.webp',
      'assets/images/عبدالقادر.webp',
    ],
    link: "",
  },

  /* ══ CREATIVE → YOUTUBE ══ */

  {
    id: 10,
    l1: "creative",
    l2: "youtube",
    thumb: "assets/images/R YouTube.webp",
    featured: true,
    num: "01",
    wide: false,
    title: "YouTube Thumbnails | محتوى ديني",
    subtitle: "YouTube Thumbnails",
    desc: "وصف قصير",
    tags: ["Social", "YouTube", "Arabic"],
    bg: "#1a1208",
    label: "PROJ",
    year: "2024",
    role: "Designer",
    client: "العميل",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/1.webp',
      'assets/images/تلاوة من سورة يوسف.webp',
      'assets/images/الوقفة الأولي.webp',
      'assets/images/جاء رمضان بالبركات.webp',
      'assets/images/5.webp',
      'assets/images/7.webp',
      'assets/images/9(3).webp',
      'assets/images/10(3).webp',
      'assets/images/11(1).webp',
    ],
    link: "",
  },

  
  {
    id: 11,
    l1: "creative",
    l2: "youtube",
    thumb: "assets/images/YouTube Portfolio.webp",
    featured: true,
    num: "02",
    wide: false,
    title: " YouTube Thumbnails MR.Abdelkader",
    subtitle: "Thumbnail System",
    desc: "وصف قصير",
    tags: ["Thumbnails", "YouTube", "CTR"],
    bg: "#1a0808",
    label: "PROJ",
    year: "2024",
    role: "Designer",
    client: "م/عبد القادر الصايغ",
    fullDesc: "وصف تفصيلي.",
    images: [
      'assets/images/التكاثر في النبات الجزء الأول.webp',
      'assets/images/التكاثر في النبات الجزء التاني.webp',
      'assets/images/ورشة الهرمونات.webp',
      'assets/images/مكونات وطبقات الغلاف الجوي.webp',
      'assets/images/الغلاف الجوي ودور العالم في استدامته.webp',
      'assets/images/صور مصغرة عبد القادر1.webp',
      'assets/images/مراجعة الباب الاول علـــــوم متكاملة.webp',
      'assets/images/ورشة الدعامة والحركة.webp',
      'assets/images/الطيات تراكيب جيولوجيا.webp',
      'assets/images/التكاثر في الإنسان الجهاز التناسلي الأنثوي.webp',
      'assets/images/التكاثر في الإنسان الجهاز التناسلي الذكري.webp',
      'assets/images/15.webp'
    ],
    link: "",
  },
  {
    id: 12,
    l1: "creative",
    l2: "youtube",
    thumb: "assets/images/Moe Speak Mokup.webp",
    featured: true,
    num: "02",
    wide: false,
    title: "YouTbe Banner Moe Speak ",
    subtitle: "Channel Branding",
    desc: "وصف قصير",
    tags: ["YoTube", "Banner", "Soical Media"],
    bg: "#1a0e08",
    label: "PROJ",
    year: "2026",
    role: "Designer",
    client: "Moe Speak",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/Moe Speak Mokup.webp',
      'assets/images/Moe Speak.webp',
    ],
    link: "",
  },

  /* ══ CREATIVE → PRINT ══ */
  {
    id: 13,
    l1: "creative",
    l2: "print",
    thumb: "assets/images/أمجاد.webp",
    featured: true,
    num: "01",
    wide: false,
    title: "مطبوعات أكاديمية أمجاد التعليمية",
    subtitle: "Print Design",
    desc: "وصف قصير",
    tags: ["Print", "Layout", "banner"],
    bg: "#101a0a",
    label: "PROJ",
    year: "2025",
    role: "Designer",
    client: "أكاديمية أمجاد التعليمية",
    fullDesc: "وصف تفصيلي.",
    images: ['assets/images/كراسة.webp',
      'assets/images/تنسيق داخلي.webp',
      'assets/images/تنسيق داخلي 1.webp',
      'assets/images/بانر1.webp',
      'assets/images/بانر (5050).webp',
      'assets/images/بانر2.webp',
      'assets/images/لافتة أكاديمة أمجاد 300150.webp',
    ],
    link: "",
  },

  /* ══ TECHNICAL → AI ══ */
  {
    id: 9,
    l1: "technical",
    l2: "ai",
    thumb: "assets/images/projects/proj1-thumb.jpg",
    num: "01",
    wide: true,
    title: "Object Detection System",
    subtitle: "Computer Vision",
    desc: "Real-time YOLOv8 detection — 94% mAP on custom dataset.",
    tags: ["PyTorch", "YOLO", "OpenCV"],
    bg: "#0a1520",
    label: "YOLO",
    year: "2024",
    role: "ML Engineer",
    client: "Personal Project",
    fullDesc:
      "نموذج كشف أجسام في الوقت الفعلي باستخدام YOLOv8 — تم تدريبه على داتاسيت مخصص بدقة 94% mAP. تم النشر باستخدام FastAPI.",
    images: [],
    link: "https://github.com/ahmedmostafatech",
  },
  {
    id: 10,
    l1: "technical",
    l2: "ai",
    thumb: "assets/images/projects/proj1-thumb.jpg",
    num: "02",
    wide: false,
    title: "Arabic Sentiment NLP",
    subtitle: "Natural Language Processing",
    desc: "BERT-based Arabic sentiment analysis — 89% accuracy.",
    tags: ["NLP", "BERT", "Arabic"],
    bg: "#0a1a15",
    label: "NLP",
    year: "2024",
    role: "ML Engineer",
    client: "Personal Project",
    fullDesc:
      "نموذج تحليل المشاعر للنصوص العربية على وسائل التواصل الاجتماعي — fine-tuned BERT بدقة 89% على الداتاسيت.",
    images: [],
    link: "",
  },

  /* ══ TECHNICAL → WEB ══ */
  {
    id: 11,
    l1: "technical",
    l2: "web",
    thumb: "assets/images/projects/proj1-thumb.jpg",
    num: "01",
    wide: true,
    title: "اسم المشروع",
    subtitle: "Web Development",
    desc: "وصف قصير",
    tags: ["HTML", "CSS", "JS"],
    bg: "#0a1420",
    label: "WEB",
    year: "2024",
    role: "Web Developer",
    client: "العميل",
    fullDesc: "وصف تفصيلي.",
    images: [],
    link: "",
  },

  /* ══ TECHNICAL → NETWORKING ══ */
  {
    id: 12,
    l1: "technical",
    l2: "networking",
    thumb: "assets/images/projects/proj1-thumb.jpg",
    num: "01",
    wide: false,
    title: "CCNA Lab Series",
    subtitle: "Networking Education",
    desc: "Arabic CCNA curriculum — packet tracer labs & video explanations.",
    tags: ["CCNA", "Networking", "Arabic"],
    bg: "#001a14",
    label: "NET",
    year: "2024",
    role: "Instructor",
    client: "YouTube / Students",
    fullDesc:
      "سلسلة شاملة لشرح مادة CCNA باللغة العربية — تشمل labs على Packet Tracer وفيديوهات شرح مفصلة لكل module.",
    images: [],
    link: "https://youtube.com/@ahmedmostafatech",
  },

  /* ══ TECHNICAL → AUTOMATION ══ */
  {
    id: 13,
    l1: "technical",
    l2: "automation",
    thumb: "assets/images/projects/proj1-thumb.jpg",
    num: "01",
    wide: false,
    title: "اسم المشروع",
    subtitle: "Automation Script",
    desc: "وصف قصير",
    tags: ["Python", "API", "Automation"],
    bg: "#1a1a0a",
    label: "AUTO",
    year: "2024",
    role: "Developer",
    client: "Personal",
    fullDesc: "وصف تفصيلي.",
    images: [],
    link: "",
  },
];

const L2_LABELS = {
  creative: {
    branding: "Branding",
    social: "Social Media",
    youtube: "YouTube",
    print: "Print",
  },
  technical: {
    ai: "AI & ML",
    web: "Web Dev",
    networking: "Networking",
    automation: "Automation",
  },
};

let currentL1 = "creative",
  currentL2 = "branding",
  currentCols = 3;

/* ══ RENDER L2 ══ */
function renderL2() {
  const el = document.getElementById("l2-filter");
  if (!el) return;
  el.innerHTML = Object.keys(L2_LABELS[currentL1])
    .map(
      (c) => `
    <button class="l2-btn${c === currentL2 ? " active" : ""}" onclick="setL2('${c}',this)">
      <span>${L2_LABELS[currentL1][c]}</span>
    </button>
  `,
    )
    .join("");
}

/* ══ RENDER CARDS ══ */
function renderCards() {
  const grid = document.getElementById("works-grid");
  if (!grid) return;
  const filtered = WORKS.filter(
    (w) => w.l1 === currentL1 && w.l2 === currentL2,
  );
  document.getElementById("count").textContent = filtered.length;
  grid.style.gridTemplateColumns = `repeat(${currentCols},1fr)`;

  if (!filtered.length) {
    grid.innerHTML =
      '<div class="empty" style="display:block">Coming soon</div>';
    return;
  }

  grid.innerHTML = filtered
    .map(
      (w, i) => `
    <div class="work-card${w.wide && currentCols >= 3 ? " wide" : ""}"
         style="animation-delay:${i * 60}ms"
         onclick="openModal(${w.id})">
<div class="card-thumb">
        ${
          w.thumb
            ? `<img src="${w.thumb}" alt="${w.title}" style="width:100%;height:100%;object-fit:cover;display:block;">`
            : `<div style="width:100%;height:100%;background:${w.bg};display:flex;align-items:center;justify-content:center;"><span style="font-family:'Bebas Neue';font-size:48px;color:rgba(255,255,255,.06);letter-spacing:4px;">${w.label}</span></div>`
        }
        <div class="card-cat">${w.subtitle}</div>
        <div class="card-overlay">
          <span class="overlay-link">View Project <span class="overlay-arrow">↗</span></span>
        </div>
      </div>
      <div class="card-info">
        <div class="card-num">${w.num} / ${String(filtered.length).padStart(2, "0")}</div>
        <div class="card-title">${w.title}</div>
        <div class="card-desc">${w.desc}</div>
        <div class="card-tags">${w.tags.map((t) => `<span class="card-tag">${t}</span>`).join("")}</div>
      </div>
    </div>
  `,
    )
    .join("");
}

/* ══ MODAL ══ */
function openModal(id) {
  const w = WORKS.find((x) => x.id === id);
  if (!w) return;

  const modal = document.getElementById("proj-modal");
  const body = document.getElementById("modal-body");

  body.innerHTML = `
    <div class="modal-header">
      <div class="modal-meta">
        <span class="modal-cat">${w.subtitle}</span>
        <span class="modal-year">${w.year}</span>
      </div>
      <h2 class="modal-title">${w.title}</h2>
      <div class="modal-tags">${w.tags.map((t) => `<span class="card-tag">${t}</span>`).join("")}</div>
    </div>

    <div class="modal-info-row">
      <div class="modal-info-cell">
        <div class="modal-info-label">Role</div>
        <div class="modal-info-val">${w.role}</div>
      </div>
      <div class="modal-info-cell">
        <div class="modal-info-label">Client</div>
        <div class="modal-info-val">${w.client}</div>
      </div>
      <div class="modal-info-cell">
        <div class="modal-info-label">Year</div>
        <div class="modal-info-val">${w.year}</div>
      </div>
      ${
        w.link
          ? `<div class="modal-info-cell">
        <div class="modal-info-label">Link</div>
        <a href="${w.link}" target="_blank" class="modal-link">View Live ↗</a>
      </div>`
          : ""
      }
    </div>

    <div class="modal-desc">${w.fullDesc}</div>

    ${
      w.images && w.images.length
        ? `
      <div class="modal-images">
        ${w.images
          .map(
            (src) => `
          <div class="modal-img-wrap">
            <img src="${src}" alt="${w.title}" loading="lazy">
          </div>
        `,
          )
          .join("")}
      </div>
    `
        : `
      <div class="modal-img-placeholder" style="background:${w.bg}">
        <span>${w.label}</span>
      </div>
    `
    }
  `;

  modal.classList.add("open");
  document.body.style.overflow = "hidden";
}

function closeModal() {
  document.getElementById("proj-modal").classList.remove("open");
  document.body.style.overflow = "";
}

document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") closeModal();
});

/* ══ SETTERS ══ */
function setL1(val, btn) {
  currentL1 = val;
  currentL2 = Object.keys(L2_LABELS[val])[0];
  document
    .querySelectorAll(".l1-btn")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
  renderL2();
  renderCards();
}
function setL2(val, btn) {
  currentL2 = val;
  document
    .querySelectorAll(".l2-btn")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
  renderCards();
}
function setGrid(n, btn) {
  currentCols = n;
  document
    .querySelectorAll(".vtbtn")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
  renderCards();
}

renderL2();
renderCards();

/* ── AUTO-OPEN PROJECT FROM URL (?project=id) ── */
const urlParams = new URLSearchParams(window.location.search);
const projectId = urlParams.get('project');
if (projectId) {
  const w = WORKS.find(x => x.id === Number(projectId));
  if (w) {
    currentL1 = w.l1;
    currentL2 = w.l2;
    document.querySelectorAll('.l1-btn').forEach(b => {
      b.classList.toggle('active', b.dataset.l1 === currentL1);
    });
    renderL2();
    renderCards();
    setTimeout(() => openModal(w.id), 200);

    /* مسح الـ query string عشان الريفرش أو الرجوع للخلف ما يفتحش المودال تاني */
    window.history.replaceState({}, '', window.location.pathname);
  }
}